<div class="w-full bg-blue-50 min-h-screen mt-16 p-4">
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div class="bg-white p-4 rounded-md">
            <div class="bg-green-500 rounded-md shadow-md px-4 py-8 mb-4">
                <div class="text-xl mb-2 text-left font-bold text-white">&#8358;10,000</div>
                <div class="text-right mb-2 text-sm uppercase text-white font-light">
                    <i class="fa fa-wallet"></i>&nbsp; AVAILABLE BALANCE
                </div>
            </div>

            <div class="bg-gray-100 rounded-md shadow-md px-4 py-8 mb-4">
                <b class="font-bold text-lg text-gray-500">Topup Wallet Now</b>
                <div class="my-4 flex justify-between items-center">
                    <div class="w-full text-center font-bold text-sm xl:text-lg mb-4 border-b-4 p-4 border-gray-400 pb-4 cursor-pointer"><a href="/account/wallet">Self-Topup</a></div>
                    <div class="bg-white text-center font-bold text-sm xl:text-lg w-full mb-4 p-4 rounded-t-md border-b-4 border-blue-600 pb-4 cursor-pointer"> Thirdparty-Topup</div>
                </div>
                <div class="mt-6 bg-gray-50 p-4 rounded-md">
                    <div class="bg-gray-100 p-4 rounded-md mb-4 border border-blue-100">
                        <p class="text-gray-600 font-bold border-b border-gray-200 pb-4">Your Wallet ID</p>
                        <div class="bg-yellow-400 p-4 text-sm  border-2 border-yellow-600">
                            Your account wallet ID - <b class="text-lg">34FER</b>, share this to recieve wallet topup 
                        </div>
                    </div>

                    <div class="bg-gray-100 p-4 rounded-md border border-blue-100">
                        <p class="text-gray-600 font-bold border-b border-gray-200 pb-4">Enter the amount you want to topup below.</p>
                       
                        <form method="" class="mt-4">
                            <p class="text-green-600 bg-green-100 p-4 font-bold mb-4">
                                Please enter amount you want to topup below and enter a thirdparty wallet address to send funds to, Note amount entered will be deducted from available wallet balance and send to wallet provided
                            </p>
                            <div class="block mb-2">
                                <input type="text" id="" class="border-3 border-gray-100 p-4 rounded-md outline-none w-full" placeholder="Enter Amount"/>
                            </div>
                            <div class="block mb-2">
                                <input type="text" id="" class="border-3 border-gray-100 p-4 rounded-md outline-none w-full" placeholder="Enter Wallet ID"/>
                            </div>
                            <div class="block mt-4">
                                <button type="submit" class="block w-full bg-blue-600 text-center text-xl font-semibold text-white p-4 rounded-md">
                                    Send Now
                                </button>
                            </div>
                        </form>
                    </div>
                </div>
            </div>

        </div>
        <div>


        <div>
            <div class="bg-gray-50 p-4 rounded-md">
                <b class="font-bold text-lg text-gray-500">Third Topup History</b>
                <ul class="overflow-x-scroll ">
                    <li class="mb-4 p-4 border-b-2 border-gray-100 flex justify-between font-bold">
                        <div>Date</div>
                        <div>Amount(&#8358;)</div>
                        <div class="hidden md:block">Remarks</div>
                        <div>Status</div>
                    </li>

                    <li class="mb-4 p-4 border-b-2 border-gray-100 flex justify-between font-bold">
                        <div>2021-04-02</div>
                        <div>&#8358;10,000</div>
                        <div class="hidden md:block">09000000645/topup</div>
                        <div>successful</div>
                    </li>
                </ul>
            </div>
        </div>
            
        </div>
    </div>
</div>
