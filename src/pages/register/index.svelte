<script>
    ...
    import {SIGN_USER_UP} from "../../actions/auth/index"
    let phone = ''
    let password = ''
    let name = ''
    onMount(() => {
            ...
            if(res){
                if(res.status == 'success' && res.proceed == 1){
                    document.querySelector("#info").innerHTML = `
                        <div class="mb-4 rounded-md bg-green-200 text-green-600 font-bold p-4 border-l-4 border-green-600">Account created successfully.</div>
                    `;
...
<div classs="mt-24 bg-blue-900 m-0 p-4">
    <form class="bg-white rounded-md shadow-md w-10/12 md:w-1/2 lg:w-1/3 m-auto mt-8 p-4 md:p-8" method="POST" on:submit|preventDefault={SIGNUP}>
    <div id="info"></div>
        ...
        <div class="block my-4 p-2">
            ...
            <div class="text-gray-600 font-normal text-center">
                Create an account with G-Intelligence transport and wallet solution it only takes a minute.
            </div>
        </div>
        <div class="block mb-2">
            <input required type="text" id="name" bind:value={name} class="border-3 border-gray-100 p-4 rounded-md outline-none w-full" placeholder="Display Name"/>
        </div>
        <div class="block mb-2">
        ...
        <p class="mt-4 p-4 block">
            <span class="text-gray-500">
                ...
                <a href="/login" class="text-blue-600 font-bold">Login Now..</a>
            </span>
        </p>
